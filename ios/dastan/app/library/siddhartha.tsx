...
/**
 * Siddhartha reader — twelve stations.
 *
 * Editorial long-form layout, matched in spirit to the Tao reader:
 *  - A curator hero opens the book (first station only).
 *  - Each station leads with its motif ornament + caption.
 *  - The German source sits as a quiet epigraph; tap to reveal the
 *    English translation beneath it.
 *  - The essay runs in sustained serif text with generous line height.
 *  - A pull quote with a gold rule; one editor's note below it.
 *  - A horizontal station strip, prev/next nav, and a colophon close.
 */

const INK = "#1E1A15";
const INK_SOFT = "rgba(30,26,21,0.88)";
const INK_MUTED = "rgba(30,26,21,0.55)";
const INK_FAINT = "rgba(30,26,21,0.28)";
const GOLD_SOFT = "rgba(184,154,91,0.55)";
const GOLD_FAINT = "rgba(184,154,91,0.14)";

export default function SiddharthaReader() {
  const [catalog, setCatalog] = useState<SiddharthaCatalog | null>(null);
...
/* ── Curator hero (first station only) ───────────────────────── */
...
function Header({
  ...
  return (
    <View style={styles.header}>
      ...
      <Text style={styles.motifOrn}>ॐ</Text>
      {scene.motif_caption ? (
...
/* ── German epigraph — source always present, tap to translate.
   The German line stands as a quiet epigraph. A tap folds the
   English translation underneath it. No pill, no modal — the
   source is always there, just waiting to be seen. */

function GermanEpigraph({
  german,
  english,
  expanded,
  onToggle,
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={expanded ? "Hide English translation" : "Show English translation"}
    >
      <Text style={[styles.epigraphLine, expanded && styles.epigraphLineFull]}>
        {german}
      </Text>
      {expanded && english ? (
...
/* ── Essay — editorial long-form paragraphs ──────────────────── */
...
/* ── Pull quote — gold rule on the left ──────────────────── */
...
/* ── Editor note — small italic aside under the pull quote ── */
...
function Nav({
  ...
  prev: SiddharthaScene | null;
  next: SiddharthaScene | null;
  onPrev: () => void;
  onNext: () => void;
}) {
  return (
...
/* ── Colophon — translator/editor credit ───────────────────── */
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.center}>
        ...
        <Text style={styles.loading}>Opening the path…</Text>
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  railWrap: {
    height: 2,
    backgroundColor: "rgba(30,26,21,0.06)",
    overflow: "hidden",
  },
  railFill: {
    height: "100%",
    backgroundColor: "rgba(30,26,21,0.65)",
  },
  ...
  stripWrap: {
    ...
    paddingTop: space.lg,
    borderTopWidth: StyleSheet.hairlineWidth,
    borderTopColor: "rgba(30,26,21,0.12)",
  },
  stripEyebrow: {
    fontFamily: fonts.uiBold,
    fontSize: 10,
  ...
  chipIdle: {
    ...
    borderWidth: 1,
    borderColor: "rgba(30,26,21,0.15)",
  },
  chipActive: {
    backgroundColor: INK,
    borderWidth: 1,
